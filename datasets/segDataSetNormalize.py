...
def PreImg(imgs_data):
    ...
    idx_0_005,idx_0_995=imgs_data_copy[round(0.005*idx)],imgs_data_copy[round(0.995*idx)]
    imgs_data=torch.where(imgs_data<idx_0_005,idx_0_005,imgs_data)
    imgs_data=torch.where(imgs_data>idx_0_995,idx_0_995,imgs_data)
...
def getTotal(dataset_path,n_classes=3):
    ...
    return PreImg(torch.FloatTensor(imgs_data)).unsqueeze(dim=1), \
...
if __name__ == '__main__':
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        print(batch_idx, data.shape, target.shape,target[0].shape,target[0].unique())
