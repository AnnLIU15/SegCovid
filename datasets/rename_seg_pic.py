...
# @Last Modified time: 2021-06-02 15:59:36
...
class BatchRenamePics(object):
    ...
    def _divide(self):
        folder=[self.save_path+'/'+var for var in os.listdir(self.save_path) if ('mask' in var) or ('img' in var)]
    ...
    def rename_divide_preprocess(self):
        folder=[self.path+'/'+var for var in os.listdir(self.path) if ('mask' in var) or ('image' in var)]
        ...
        for type_img in folder:
            ...
            for root, _, files in os.walk(type_img):
                ...
                for var in name_list:
                    ...
                    >>> mask/19_84.png
                    '''
                    shutil.copyfile(var,save_dir+'/'+var[len(type_img)+1:][::-1].replace('/','_',1)[::-1]) 
        self._divide()
    
if __name__ == '__main__':
    # 设置起始路径path
    parser_=argparse.ArgumentParser()
    parser_.add_argument('--path','-p',type=str,default='/home/e201cv/Desktop/dataset/nCoVR/ct_lesion_seg',help='unzip ct_lesion_seg.zip path')
    parser_.add_argument('--save_path','-s',type=str,default='/home/e201cv/Desktop/dataset/nCoVR/mytest',help='save rename pic')
    parser_.add_argument('--train_txt',type=str,default='./data/train_split_segmentation.txt',help='save rename pic')
    parser_.add_argument('--text_txt',type=str,default='./data/test_split_segmentation.txt',help='save rename pic')
    args=parser_.parse_args()
    # 创建实例对象
    pics = BatchRenamePics(args.path,args.save_path,args.train_txt,args.text_txt)
    # 调用实例方法
