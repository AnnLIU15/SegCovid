...
def main(args):
    device, lrate, num_classes, num_epochs, log_name, batch_size =\
        args.device, args.lrate, args.num_classes, args.num_epochs, args.log_name, args.batch_size
    ...
    if log_name==None:
        writer=SummaryWriter('./log/seg/'+time.strftime('%m%d-%H%M',time.localtime(time.time())))
    else:
